...
4. Readability & Structure: Format your response using clean, step-by-step bullet points and headers that are easily human-understandable. Avoid large blocks of prose.
5. Output Format: You MUST respond with a valid JSON object in the following format:
{
  "answer": "Your markdown answer here. Keep it detailed, highly structured, and use inline citations. Use code blocks for code snippets.",
  "citations": [
    {
...
SYSTEM_PROMPT_STREAM = """You are RepoSage, an expert developer assistant specialized in codebase Q&A.
Your task is to answer the user's question about the repository based ONLY on the provided source code chunks.
...
2. Incomplete Context: If the provided code chunks do not contain enough information to answer the question, state: "I don't have enough context in the indexed files to answer this question."
3. Citations: When you reference code, files, or logic, cite them inline in your answer using the format `[filename:Lstart-lend]` (e.g. `[main.py:L10-25]`).
4. Readability & Structure: Format your response using clean, step-by-step bullet points, lists, and headers that are easily human-understandable. Avoid large blocks of prose.
5. Output Format: Write your response in plain Markdown. Do NOT wrap it in JSON. Start directly with the answer. Do not add any JSON formatting or wrappers.
"""

def build_rag_messages_stream(query: str, chunks: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    """
    Assembles context chunks and user query for streaming markdown response.
    """
    context_parts = []
    for idx, chunk in enumerate(chunks):
        meta = chunk["metadata"]
    ...
    user_prompt = (
        ...
        f"======================\n\n"
        f"User Question: {query}\n\n"
        f"Generate the markdown answer, citing inline."
    )
